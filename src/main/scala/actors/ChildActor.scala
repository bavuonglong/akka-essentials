  ...
  class WordCounterMaster extends Actor {
    ...
    override def receive: Receive = {
      case Initialize(nChildren) =>
        ...
        val childrenRefs = for (i <- 1 to nChildren) yield context.actorOf(Props[WordCounterWorker], s"wcm_$i")
    ...
    def withChildren(childrenRefs: Seq[ActorRef], currentChildIndex: Int, currentTaskId: Int, requestMap: Map[Int, ActorRef]): Receive = {
      case text: String =>
        println(s"[mater] I have received: $text - I will send it to cloud $currentChildIndex")
